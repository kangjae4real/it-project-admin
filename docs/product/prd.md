...
| 2 | 전체 팀을 목록으로 보고 검색·필터하고 싶다 | 팀명·인원수·구성 학과·소속 리그 표시, 팀명 검색, 학과·리그 필터 |
| 3 | 팀을 눌러 팀원 상세를 보고 싶다 | 팀 정보와 소속 팀원 목록(이름·학번·학과·중도하차 여부) |
| 4 | 전체 팀원을 한 목록에서 보고 싶다 | 이름·학번·학과·소속팀·중도하차 표시, 이름/학번 검색, 학과·팀 필터 |
| 5 | 팀·팀원·학과·리그를 추가·수정·삭제하고 싶다 | 각 엔티티 CRUD, 폼 검증, 소프트삭제 |
| 6 | 팀원의 중도하차를 표시하고 싶다 | 중도하차 여부 토글, 목록에서 구분 표시 |
| 7 | 기존 데이터를 앱으로 옮기고 싶다 | export를 import(seed)하는 스크립트, 재실행해도 결과가 같음(upsert) |

**P1 (권장)**

| # | User Story | 완료 조건 |
|---|-----------|-----------|
| 8 | 전체 현황을 요약으로 보고 싶다 | 대시보드: 총 팀 수·총 인원, 학과별 인원 분포 |
| 9 | 사용 중인 팀/학과 삭제 시 실수를 막고 싶다 | FK 제약으로 차단, "먼저 팀원 재배치" 안내 |

**P2 (선택/향후)**

| # | User Story | 완료 조건 |
|---|-----------|-----------|
| 10 | 누가 언제 무엇을 바꿨는지 남기고 싶다 | 감사 로그(변경 이력) |
| 11 | 심사·점수·상태를 관리하고 싶다 | v2, 회의 후 결정 |
| 12 | 비밀번호를 직접 바꾸고 싶다 | 셀프 비밀번호 변경 |
